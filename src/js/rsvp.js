...
function getRSVP() {
  ...
  if (!guestName) {
    $("#submitError").empty();
    $("#submitError").append(
      "<span class='text-danger'>Oops! Looks like you forgot to fill in your name.</span>"
    );
  } else {
    ...
    $.ajax({
      url:
        "https://h4467dph4g.execute-api.us-west-2.amazonaws.com/develop/Guest?guest_name=" +
        guestName,
      context: this,
      cache: false,
      method: "GET",
      ...
      error: function(data) {
        ...
        $("#submitError").empty();
        $("#submitError").append(
          "<span class='text-danger'>Something went wrong. Please try again later.</span>"
        );
      }
    });
  }
...
function displayRSVP(data) {
  ...
  if (submissionChk) {
    let submissionBtn = $(
      '<div class="form-group"><button type="submit" id="rsvpSubmitBtn" class="btn btn-default btn-lg">Submit</button></div>'
    );
...
function renderRSVPFormCard(data, i) {
  ...
  let top = $(
    "<div class='form-row guest text-left'><input class='_id' type='hidden' value='" +
      data._id +
      "'><h3 class='guest-name'>" +
      data.Guest +
      "</h3></div>"
  ...
  let underline = $(
    "<div class='row'><div class='col-sm-4 col-xs-6 col marker'></div><div class='col-sm-7 col-xs-6'></div></div>"
  );
  ...
  let respY = $(
    "<input type='radio' class='form-check-input response' name='rsvp" +
      i +
      "' id ='rsvpYes" +
      i +
      "'" +
  ...
  let labelY = $(
    "<label class='form-check-label' for='rsvpYes" +
      i +
      "'" +
      "> Accepts With Pleasure</label>"
  );
  let respN = $(
    "<input type='radio' class='form-check-input response' name='rsvp" +
      i +
  ...
  let labelN = $(
    "<label class='form-check-label' for='rsvpNo" +
      i +
      "'" +
      "> Regretfully Declines</label>"
  );
  let dietDiv = $(
    "<div class='form-check text-left'><h5>Let us know if you have any dietary restrictions<h5></div>"
  );
  let noRestrictions = $(
    "<div class='form-row'><input type='radio' class='form-check-input meal' name='meal" +
      i +
      "' id='noRs" +
      i +
      "' + value='0'>" +
      "<label class='form-check-label' for='noRs" +
      i +
      "'" +
      "> No Restrictions</label></div>"
  );
  let veg = $(
    "<div class='form-row'><input type='radio' class='form-check-input meal' name='meal" +
      i +
  ...
  let celiac = $(
    "<div class='form-row'><input type='radio' class='form-check-input meal' name='meal" +
      ...
      i +
      "' + value='3'>" +
      "<label class='form-check-label' for='celiac" +
      i +
      "'" +
      ">Gluten Free</label></div>"
  );
  let nuts = $(
    "<div class='form-row'><input type='radio' class='form-check-input meal' name='meal" +
      i +
  ...
  let other = $(
    "<div class='form-row'><input type='radio' class='form-check-input meal' name='meal" +
      ...
      i +
      "'" +
      ">Other (Please provide details in text box below)</label></div>"
  );
  let textRow = $(
    "<div class='form-row rsvp-row text-left'><p style='color:#891632'>Please use this space to let us know if you have any dietary restrictions. Or just leave us a message!</p></div>"
  );
  let textArea = $(
    "<textarea class='form-control detailsTextArea' name='detailsTextArea" +
      i +
      "' id='detailsTextArea" +
      i +
      "' rows='3'></textarea>"
...
function renderRSVPDetailCard(data, i) {
  ...
  let top = $(
    "<div class='form-row guest'><div class='_id' style='display:none;'>" +
      data._id +
      "</div><h3 class='guest-name'>" +
      data.Guest +
      "</h3></div>"
  );
  let underline = $(
    "<div class='row'><div class='col-sm-4 col marker'></div><div class='col-sm-7'></div></div>"
  );
  let response = $(
    "<div class='row submission'><h4>" +
      "Response:<span style='color:#891632;'> " +
      data.Response +
      "</span></h4></div>"
  );
  let meal = $(
    "<div class='row submission'><h5>" +
      "Dietary Restrictions:</h5><p> " +
      mealMap[data.Diet] +
      "</p></div>"
  );
  let details = $(
    "<div class='row submission'><h5>Details:</h5><p>" +
      data.Details +
      "</p></div>"
  );
...
function renderResponseConfirmation(data) {
  ...
  let response = $(
    "<div class='row' style='padding-left: 15px;'><p><strong>Response: </strong>" +
      data.response +
      "</p></div>"
  );
  let meal_choice = $(
    "<div class='row' style='padding-left: 15px;'><p><strong>Meal Choice: </strong>" +
      mealMap[data.meal] +
      "</p></div>"
  );
...
function appendUpdateResponseBtn() {
  let btn = $(
    "<div class='col-sm-12 text-center'><button type='submit' id='updateResponse' class='btn btn-default btn-lg'>Update Response</button></div>"
  );
